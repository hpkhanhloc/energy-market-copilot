...
def render_facts(inv: Investigation) -> str:
    ...
    end = (e.end.tz_convert(TZ)).strftime("%H:%M") if e.hours > 1 else None
...
def fallback_narrative(inv: Investigation) -> Narrative:
    ...
    summary = (
        f"A {KIND_TEXT[e.kind]} of {e.peak_price:,.0f} EUR/MWh against a baseline of "
        f"{e.baseline_median:,.0f} EUR/MWh. Evidence is consistent with: {lead}. "
...
SMALL_COUNT_LIMIT = 31  # hours, days, counts of drivers: allowed without a matching fact
...
def unknown_numbers(narrative: Narrative, facts: str) -> list[str]:
    ...
    for text in _texts(narrative):
        for raw in NUMBER.findall(text):
            ...
            if value in allowed:
                continue
            if not value.endswith("%") and _small_count(value):
                continue
...
def results_as_text(results: list[DriverResult]) -> str:
    return "\n".join(
        f"- [{r.verdict}] {r.title}. Hypothesis: {r.hypothesis} Evidence: {r.detail}"
        for r in results
    )
